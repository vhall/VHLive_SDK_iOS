...
@end
@implementation VHChangeSegmentView
...
-(void)initData{
    ...
    if (_titles.count<MAX_TitleNumInWindow+1) {
        _btn_w=VHSegmentViewWidth/_titles.count;
    }else{
        _btn_w=VHSegmentViewWidth/MAX_TitleNumInWindow;
    }
    ...
    _titleSelectColor=[UIColor colorWithHex:@"#222222"];
}
...
-(void)btnClick:(UIButton *)btn{
    ...
    CGFloat maxOffsetX= _bgScrollView.contentSize.width-VHSegmentViewWidth;
...
//    NSLog(@"====%f",offsetx);
    for (UIButton *btn in _btns) {
        ...
        if (btn.tag-1==_defaultIndex-1) {
            ...
            CGFloat nextw=0;
            if (offsetx<(btn.tag-1)*VHSegmentViewWidth) {
                NSLog(@"向左");
                nextw = [self.titlesStrWidthArray[btn.tag-2] floatValue];
                CGFloat max_w = (_btn_w*2 + w +nextw)/2;
                CGFloat endoffsetx = (btn.tag-1)*VHSegmentViewWidth-offsetx;
                ...
                CGFloat max_w = (_btn_w*2 + w +nextw)/2;
                CGFloat endoffsetx = offsetx - (btn.tag-1)*VHSegmentViewWidth;
                if (_selectLine.layer.width<max_w) {
                    _selectLine.layer.width=w+MIN(endoffsetx, max_w);
                }else{
                    _selectLine.layer.width=max_w;
...
-(void)updateView{
    for (UIButton *btn in _btns) {
        ...
        [btn setTitleColor:_titleSelectColor forState:UIControlStateSelected];
        btn.titleLabel.font=_titleFont;
        _selectLine.backgroundColor=[UIColor colorWithHex:@"#FB3A32"];
...
-(UIScrollView *)bgScrollView{
    if (!_bgScrollView) {
        _bgScrollView=[[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, VHSegmentViewWidth, self.frame.size.height)];
