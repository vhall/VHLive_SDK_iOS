...
- (IBAction)QAButtonClick:(UIButton *)sender {
    ...
    if (!_isQuestion_status) {
        NSString *close = [NSString stringWithFormat:@"主播关闭了%@",self.questionDesc];
        VH_ShowToast(close);
        return;
    }
...
- (void)onlineChangeRealNum:(NSUInteger)online_real virtualNum:(NSUInteger)online_virtual {
    if(self.moviePlayer.webinarInfo.online_show) {
...
- (void)Announcement:(NSString*)content publishTime:(NSString*)time
{
    ...
    // 添加公告控件
    [self addAnnouncementBtn];
    // 如果有公告列表并且展示中
    if (_announcementList && _announcementList.isShow) {
        // 隐藏小红点
...
- (void)clickAnnouncementListToBtn
{
    VHLog(@"点击公告列表");
    // 隐藏小红点
    [self.announcementListBtn hidenBadge];

    [self.view addSubview:self.announcementList];
...
- (void)vhallQAndADidOpened:(VHallQAndA *)QA
{
    ...
    [self.questionName setTitle:self.questionDesc forState:UIControlStateNormal];
    NSString *open = [NSString stringWithFormat:@"主持人开启了%@",self.questionDesc];
    VH_ShowToast(open);
...
- (UIButton *)announcementListBtn
{
    if (!_announcementListBtn) {
        _announcementListBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [_announcementListBtn setImage:BundleUIImage(@"vh_an_btnIcon") forState:UIControlStateNormal];
