...
@property (nonatomic, assign) NSInteger applicable_product_type;    ///<适用sp类型 0-全部sp 1-指定sp可用 2-指定sp不可用
...
/// 通过CDN地址拉取活动列表，推荐
/// - Parameters:
+(void)goodsGetOnlineListFromCDN:(NSString*)url  complete:(void (^)(NSArray <VHGoodsListItem *> *list, NSError *error))complete;
...
///   - success: 成功
///   - fail: 失败
+ (void)goodsOrderNotice:(NSString *)webinar_id user_id:(NSString*)user_id third_user_id:(NSString *)third_user_id username:(NSString *)username goods_id:(NSString *)goods_id act:(NSString*)act  notice_type:(NSString *)notice_type success:(void (^)(NSString *msg))success fail:(void (^)(NSError *error))fail;
...
/// - Parameter url: 创建url对象进行跳转
/// - Parameter scheme: app配置的scheme,
/// - Parameter referer:
