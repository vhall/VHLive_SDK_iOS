...
- (void)configure{
    ...
    [self initTimerCompletion:^{
        if ((self.precision < 10) || (self.precision > 1000)) {
            @throw [NSException exceptionWithName: @"VHTimer-精度异常" reason: @"precision参数配置异常，大于1000毫秒的计时回调将无意义，以及小于10毫秒无实际意义" userInfo: nil];
        }
        if (!self.isAscend && self.timerInterval <= 0) {
            @throw [NSException exceptionWithName: @"VHTimer-参数：timerInterval配置失败" reason:@"倒计时模式下开始时时间不能为小于或等于0" userInfo: nil];
        }
        VHTime *progress = [self timeProgress: self.currentTime];
        if (self.isAscend) {
            self.currentTime = [self value: self.currentTime byOpration: VHDecimalOprationTypeAdd percision:6 withValue: self.onceTime].doubleValue;
            ...
            self.currentTime = [self value: self.currentTime byOpration: VHDecimalOprationTypeSubtract percision: 6 withValue: self.onceTime].doubleValue;
