...
- (void)setUpMasonry
{
    ...
    [self.userTitleLab mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.img.mas_bottom).offset(27);
...
- (void)showGoodsOrder:(VHGoodsListItem *)item settingItem:(VHGoodsSettingItem *)settingItem webinarInfo:(VHWebinarInfo *)webinarInfo
{
    ...
    CGFloat price = [[VUITool isBlankString:item.discount_price] ? item.price : item.discount_price floatValue];
    self.totalPrice = self.numberBtn.currentNumber * price;
    
    NSString * totalText = [NSString stringWithFormat:@"合计¥%@", [NSString stringWithFormat:@"%.2f",self.totalPrice]];
