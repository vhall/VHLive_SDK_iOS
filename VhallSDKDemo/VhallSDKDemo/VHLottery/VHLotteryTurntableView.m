...
- (void)showLotteryTurntableWithVHLottery:(VHallLottery *)vhLottery startModel:(VHallStartLotteryModel *)startModel
{
    ...
    [self.iconImg sd_setImageWithURL:[NSURL URLWithString:imgStr]
                    placeholderImage:[UIImage imageNamed:@"正在加载"]
    ...
    otherInfo[@"type"] = @(startModel.type);
    [VUITool sendTestsNotificationCenterWithKey:VHTests_Lottery_Start otherInfo:otherInfo];
...
- (void)clickSendBtn
{
    [self.vhLottery lotteryParticipationWithRoomId:self.startModel.huadieInfo.room_id
                                        lottery_id:self.startModel.huadieInfo.lottery_id
                                           command:self.startModel.huadieInfo.command
                                           success:^{
...
- (UIButton *)sendBtn {
    if (!_sendBtn) {
        ...
        _sendBtn.accessibilityLabel = VHTests_Lottery_SendCode;
        _sendBtn.hidden = YES;
