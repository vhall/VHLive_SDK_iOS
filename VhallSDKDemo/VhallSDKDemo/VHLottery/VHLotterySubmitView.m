...
- (void)showLotterySubmitWithVHLottery:(VHallLottery *)vhLottery endLotteryModel:(VHallEndLotteryModel *)endLotteryModel submitList:(NSArray <VHallLotterySubmitConfig *> *)submitList
{
    ...
    [self.vhLottery lotteryWinningUserInfoWithRoomId:endLotteryModel.huadieInfo.room_id
                                            complete:^(VHallLotteryUserInfo *userInfo, NSError *error) {
        ...
        [VUITool sendTestsNotificationCenterWithKey:VHTests_Lottery_FillInSubmit otherInfo:otherInfo];
    }];
...
- (void)clickSubmitBtn
{
    ...
    __weak __typeof(self) weakSelf = self;
    [self.vhLottery submitLotteryInfo:param
                              success:^{
...
- (UIButton *)submitBtn {
    if (!_submitBtn) {
        ...
        _submitBtn.accessibilityLabel = VHTests_Lottery_SubmitBtn;
